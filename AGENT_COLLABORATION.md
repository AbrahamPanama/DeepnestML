...
| Raster collision tier (RC-1 … RC-5) | `docs/raster-collision-plan.md` | Written 2026-07-25 by Claude-Code. Answers "why can't we have 1-degree rotations": outer NFPs are keyed per rotation pair, so distinct-NFP count grows as (sources x rotations)^2 — a 1-degree grid on albano is 74.6M NFPs (~41 h), unreachable by construction. Adds a second collision tier (raster bitmasks, O(R) not O(R^2)) as a two-sided *sound* filter during search, with exact geometry retained as the only commit-time authority. Standard architecture in production nesters that appear to rotate freely. RC-1/RC-2 are an explicit decision point: if the ambiguity rate on slender parts is poor, stop and report. Claim RC-x from the plan's §8 table |
