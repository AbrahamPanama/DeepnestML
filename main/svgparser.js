 ...
 */
 ...
 (function(root){
	...
	SvgParser.prototype.cleanInput = function(dxfFlag){
		// Inline authored styles before flatten/filter strip the <style>/<defs> context.
		this.inlineAuthorStyles(this.svgRoot);
	...
	SvgParser.prototype.inlineAuthorStyles = function(root){
		if(!root){
			return;
		}
		...
		for(var i=0; i<styleProperties.length; i++){
			inheritableProperties[styleProperties[i]] = true;
		}

		var parseDeclarations = function(styleText){
			var declarations = {};
			if(!styleText){
				return declarations;
			}
			...
			styleText.split(';').forEach(function(chunk){
				...
				var property = chunk.substring(0, separatorIndex).trim().toLowerCase();
				var value = chunk.substring(separatorIndex+1).trim();
				if(styleProperties.indexOf(property) < 0 || !value){
					return;
				}
				declarations[property] = value.replace(/\s*!important\s*$/i, '').trim();
			});
		...
		var parseSelector = function(selector){
			...
			// Keep V1 intentionally simple and safe: support tag/class/id combinations only.
			...
			while(selector.length > 0){
				if(selector.charAt(0) === '.'){
					...
					selector = selector.substring(classMatch[0].length);
				}
					...
					id = idMatch[1];
					selector = selector.substring(idMatch[0].length);
				}
				else{
					return null;
				}
		...
		var matchesSelector = function(element, selector){
			...
			if(selector.id && element.getAttribute('id') !== selector.id){
				return false;
			}

			if(selector.classes.length > 0){
				var className = element.getAttribute('class') || '';
				var classList = className.trim() ? className.trim().split(/\s+/) : [];
				for(var i=0; i<selector.classes.length; i++){
					if(classList.indexOf(selector.classes[i]) < 0){
						return false;
					}
				}
			}
		...
		var styleRules = [];
		var styleNodes = root.querySelectorAll ? root.querySelectorAll('style') : [];
		Array.prototype.forEach.call(styleNodes, function(styleNode){
			var cssText = styleNode.textContent || '';
			cssText = cssText.replace(/\/\*[\s\S]*?\*\//g, '');
			var ruleRegex = /([^{}]+)\{([^{}]+)\}/g;
			var ruleMatch = null;
			while((ruleMatch = ruleRegex.exec(cssText))){
				var selectors = ruleMatch[1].split(',');
				var declarations = parseDeclarations(ruleMatch[2]);
				selectors.forEach(function(selectorText){
					...
					styleRules.push({
						...
						order: styleRules.length
					});
				});
			}
		});

		var applyStyles = function(element, inheritedStyles){
			...
			for(property in inheritedStyles){
				if(inheritedStyles.hasOwnProperty(property)){
					authored[property] = inheritedStyles[property];
					resolved[property] = inheritedStyles[property];
				}
			}

			for(var ruleIndex = 0; ruleIndex < styleRules.length; ruleIndex++){
				var rule = styleRules[ruleIndex];
				if(!matchesSelector(element, rule.selector)){
					continue;
				}
				for(property in rule.declarations){
					if(!rule.declarations.hasOwnProperty(property)){
						continue;
					}
					var state = propertyState[property];
					if(!state || rule.selector.specificity > state.specificity || (rule.selector.specificity === state.specificity && rule.order >= state.order)){
						resolved[property] = rule.declarations[property];
						authored[property] = rule.declarations[property];
			...
			for(var propIndex = 0; propIndex < styleProperties.length; propIndex++){
				property = styleProperties[propIndex];
				if(element.hasAttribute(property)){
					resolved[property] = element.getAttribute(property);
					authored[property] = resolved[property];
				}
			}

			var inlineDeclarations = parseDeclarations(element.getAttribute('style'));
			for(property in inlineDeclarations){
				if(inlineDeclarations.hasOwnProperty(property)){
					resolved[property] = inlineDeclarations[property];
					authored[property] = inlineDeclarations[property];
				}
			}

			if(styledElements[element.tagName ? element.tagName.toLowerCase() : '']){
				for(property in resolved){
					if(resolved.hasOwnProperty(property) && resolved[property] !== null && typeof resolved[property] !== 'undefined' && resolved[property] !== ''){
						element.setAttribute(property, resolved[property]);
					}
				}
			}
			...
			for(property in authored){
				if(authored.hasOwnProperty(property) && inheritableProperties[property]){
					childInherited[property] = authored[property];
				}
			}

			var children = Array.prototype.slice.call(element.children || []);
			for(var childIndex = 0; childIndex < children.length; childIndex++){
				applyStyles(children[childIndex], childInherited);
			}
		};
